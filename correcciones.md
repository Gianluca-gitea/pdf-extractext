...
> **Revisión 2026-09-09:** verificados los puntos **1, 3 y 4** — aprobados y eliminados (config centralizada en `Settings` con inyección en `DocumentRepository`; `load_dotenv()` movido a `run_dev.py`). El punto **2** sigue pendiente (sin fail-fast) y se actualizó. La numeración original se conserva.

---

## Configuración y Entorno (12-Factor App)

### 2. Configuración sin default seguro en el repo (`app/settings.py`)
- **Archivo:** `app/settings.py` (default de `mongodb_uri`)
- **Severidad:** Media
- **Descripción:** Tras la centralización (revisión 2026-09-09) el fallback `mongodb://localhost:27017` vive en un solo lugar (`Settings`), pero un contenedor o entorno sin `MONGODB_URI` sigue intentando conectar a `localhost`, lo que en producción falla silenciosa lentamente.
- **Corrección:** Dejar la URI como obligatoria en entornos no-dev o fallar rápido (fail-fast) si no está configurada, en vez de asumir localhost.
...
- **Media:** 2, 6, 7, 8, 9, 13, 15, 16, 17, 20, 22, 27, 30 (13 ítems)
- **Baja:** 5, 10, 11, 12, 14, 18, 19, 21, 23, 24, 25, 26, 28, 29, 31

**Prioridad de acción sugerida:**
1. Fail-fast de configuración Mongo en entornos no-dev (2).
2. Inyección de dependencias y ciclo de vida de Mongo (16, 17).
3. Refactorizar `interface.py` (desacople + URL + bug 20/21/22/26).
4. Limpiar código muerto y consolidar dependencias (6, 13, 12).
5. Unificar estados en un enum (9, 25).
